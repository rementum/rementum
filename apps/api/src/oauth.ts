    ...
    features: {
      ...
      resourceIndicators: {
        ...
        getResourceServerInfo: (_ctx, indicator) => {
          if (
            indicator !== resource &&
            indicator !== `${config.REMENTUM_PUBLIC_URL.replace(/\/$/, "")}/api`
          ) {
...
function interactionPage(prompt: string, action: string, uid: string, body: string): string {
  ...
  :root{color-scheme:dark;--canvas:#101815;--surface:#17211e;--raised:#1c2924;--text:#f3f5f1;--muted:#bdcbc5;--quiet:#8fa099;--line:rgb(243 245 241 / 12%);--danger:#e2b6b6;--brand:#79aa98}
  *{box-sizing:border-box}body{min-width:320px;min-height:100dvh;margin:0;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at 50% 15%,rgb(255 255 255 / 5%),transparent 32%),var(--canvas);color:var(--text);font:14px/1.5 ui-sans-serif,system-ui,-apple-system,sans-serif}
  ...
  .brand{display:flex;align-items:center;gap:10px;margin-bottom:34px;color:var(--muted);font-size:13px;font-weight:650}.mark{display:block;width:28px;height:28px;flex:0 0 28px}
