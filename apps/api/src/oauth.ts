...
export async function buildOauthRuntime(
  ...
  const configuration: Configuration = {
    ...
    // MCP connection consent is persistent even when the client omits OIDC offline_access.
    // Match refresh issuance so browser session expiry cannot invalidate these tokens.
    expiresWithSession: (ctx) => !ctx.oidc.client?.grantTypeAllowed("refresh_token"),
    ttl: {
      AccessToken: 15 * 60,
...
export async function registerOauthRoutes(
  ...
  runtime.provider.on("grant.error", (ctx, error) => {
    ...
    app.log.warn(
      { event: "oauth_refresh_failed", reason: oauthRefreshFailureReason(error) },
...
// Provider details can contain caller-controlled values. Log only known diagnostic categories.
export function oauthRefreshFailureReason(error: unknown): string {
  if (!(error instanceof errors.InvalidGrant)) return "other";
  switch (error.error_detail) {
    case "grant is expired":
      return "grant_expired";
    case "grant not found":
      return "grant_missing";
    case "refresh token already used":
