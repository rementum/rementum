...
- Confirm `OWL_PUBLIC_URL` uses the same HTTPS origin as `OWL_DOMAIN`.
- Keep `OWL_DEV_AUTH=false` in production.
...
- Escrow `OWL_MASTER_KEY` outside the server and outside database backups.
...
Owl Memory encrypts article content at the application layer. PostgreSQL still stores routing
...
the age identity apart from the archives. A valid archive without `OWL_MASTER_KEY` cannot decrypt
article bodies.
